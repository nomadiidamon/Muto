...
namespace Muto {
	...
	void EditorLayer::OnAttach()
	{
		...
		// Scene setup
		{
			...
			if (MU_EDITOR_USE_DEFAULT_SCENE) {
				...
				if (!valid) {
					MU_CORE_ERROR("Failed to load default scene: " + loadedScene);
					MU_CORE_ERROR("Attempted Scene: " + std::string(MU_EDITOR_DEFAULT_SCENE));
					MU_CORE_ERROR("Current Working Directory: " + FileSystem::GetCurrentWorkingDirectory());
					MU_CORE_ERROR("Absolute Path Attempted: " + FileSystem::GetAbsolutePath(loadedScene));
					MU_CORE_ERROR("Error loading the scene, please check the paths and file availability.");
				}
				else {
					MU_CORE_INFO("Successfully loaded the default scene: " + loadedScene);
				}
			}
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		// Resize
		if (Muto::FramebufferSpecification spec = m_Framebuffer->GetSpecification();
		...
		{
			{
				MU_PROFILE_SCOPE("Entity Scene Update");
				// Update scene
...

		{
			...
			if (selectedEntity && m_GizmoType != -1)
			{
				...
				if (ImGuizmo::IsUsing())
				{
					...
					Muto::Math::DecomposeTransform(transform, translation, rotation, scale);
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(MU_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
	...
	void EditorLayer::OpenScene()
	{
		...
		if (!filePath.empty())
		{
			...
			MU_CORE_INFO("Scene deserialized from: " + filePath);
		}
	}
	...
	void EditorLayer::SaveSceneAs()
	{
		...
		if (!filePath.empty())
		{
			...
			MU_CORE_INFO("Scene serialized to: " + filePath);
		}
	}

	void EditorLayer::ResetScene()
	{
		MU_CORE_ASSERT(false, "Not implemented yet!");
	}
